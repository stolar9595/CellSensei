...
**API Design**: RESTful endpoints handle speed test data collection, network information tracking, and cell tower management. The API supports filtering by carrier and provides pagination for large datasets.

**Storage Layer**: The application implements an abstraction layer (`IStorage` interface) that currently uses an in-memory storage implementation (`MemStorage`) for development and testing. This design allows for easy migration to persistent database storage without changing the business logic.

## Data Models and Schema
The application uses **Drizzle ORM** with **Zod** for type-safe database interactions and validation. Three main data models are defined:
...
The application is configured to use **PostgreSQL session storage** with **connect-pg-simple**, preparing for user authentication and personalized data tracking in future iterations.
